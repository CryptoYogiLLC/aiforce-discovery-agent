...
APP_SERVERS = [
    ...
    {"image": "python:3.11-slim", "ports": ["8000"], "name": "django", "lang": "python"},
    {"image": "node:20-slim", "ports": ["3000"], "name": "express", "lang": "node"},
    {"image": "node:20-slim", "ports": ["3000"], "name": "nextjs", "lang": "node"},
    {"image": "openjdk:17-slim", "ports": ["8080"], "name": "springboot", "lang": "java"},
    {"image": "openjdk:17-slim", "ports": ["8080"], "name": "quarkus", "lang": "java"},
    {"image": "mcr.microsoft.com/dotnet/aspnet:8.0", "ports": ["5000"], "name": "dotnet", "lang": "dotnet"},
    {"image": "ruby:3.2-slim", "ports": ["3000"], "name": "rails", "lang": "ruby"},
    {"image": "golang:1.21-alpine", "ports": ["8080"], "name": "goapi", "lang": "go"},
]

DATABASES = [
    {"image": "postgres:16", "ports": ["5432"], "name": "postgresql", "type": "relational"},
    {"image": "postgres:15", "ports": ["5432"], "name": "postgresql15", "type": "relational"},
    ...
    {"image": "memcached:alpine", "ports": ["11211"], "name": "memcached", "type": "cache"},
    {"image": "elasticsearch:8.11.0", "ports": ["9200", "9300"], "name": "elasticsearch", "type": "search"},
    {"image": "cassandra:4", "ports": ["9042"], "name": "cassandra", "type": "wide-column"},
...
DEPARTMENT_NAMES = [
    "erp", "crm", "hrms", "finance", "inventory", "analytics",
    "billing", "logistics", "procurement", "manufacturing",
    "warehouse", "ecommerce", "marketing", "support", "legacy"
]

COMPANY_PREFIXES = [
    "acme", "globex", "initech", "umbrella", "waynetech",
    "starkindustries", "oscorp", "lexcorp", "cyberdyne", "tyrell"
]
...
def generate_environment():
    ...
    if num_queues > 0:
        selected_queues = random.sample(MESSAGE_QUEUES, min(num_queues, len(MESSAGE_QUEUES)))
    ...
    if num_infra > 0:
        selected_infra = random.sample(INFRASTRUCTURE, min(num_infra, len(INFRASTRUCTURE)))
...
def generate_manifest(compose, seed):
    ...
    manifest = {
        ...
        "summary": {
            ...
            "web_servers": sum(1 for s in services.values() if s.get("labels", {}).get("discovery.type") == "web-server"),
            "app_servers": sum(1 for s in services.values() if s.get("labels", {}).get("discovery.type") == "app-server"),
            "databases": sum(1 for s in services.values() if s.get("labels", {}).get("discovery.type") == "database"),
            "message_queues": sum(1 for s in services.values() if s.get("labels", {}).get("discovery.type") == "message-queue"),
            "infrastructure": sum(1 for s in services.values() if s.get("labels", {}).get("discovery.type") == "infrastructure"),
        },
        "services": []
    }
...
def main():
    ...
    with open(compose_path, "w") as f:
        f.write(f"# Auto-generated test environment\n")
        f.write(f"# Seed: {seed}\n")
        f.write(f"# Generated: {datetime.now().isoformat()}\n")
        f.write(f"# Regenerate with: python scripts/generate-test-env.py\n")
        f.write(f"#\n")
        f.write(f"# To recreate this exact environment, use:\n")
    ...
    print(f"\nTo scan the network:")
    print(f"  Network range: 172.28.0.0/24")
    print("=" * 60)
